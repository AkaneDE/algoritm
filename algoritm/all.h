...
class all//класс соединяющий все элементы
{
private:
	//вектора
	vector<float> v1;
	...
	void get_Del()//нахождение разницы отсчётов
	{
		...
		for (int lag = 0; lag < lag_max; lag++) {//между первым и вторым
			if (corr(v1, v2, lag) > R)
			{
				R = corr(v1, v2, lag);
				d1 = lag;
		...
		for (int lag = 0; lag < lag_max; lag++) {//между первым и третьим
		...
		for (int lag = 0; lag < lag_max; lag++) {//между вторым и третьим
			if (corr(v2, v3, lag) > R)
			{
				R = corr(v2, v3, lag);
				d3 = lag;
	...
	vector<float> use_filter(vector<float> v)//применение фильтра к вектору
	{
...
public
	...
	void TDoA()//применение всего алгоритма
	{
		//кто сначала поиск отсчётов или филтр???
		get_Del();//поиск отсчётов
		//фильтрация
		v1 = use_filter(v1);
		v2 = use_filter(v2);
